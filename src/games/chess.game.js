...
class Chess extends Game {
  ...
  getInitialSymbol(player1, player2) {
    ...
    if (symbol1 === "w") this.currentPlayer = player1;
  ...
  makeMove(player, piece, moveIdx, currPiecePos) {
    if (player !== this.currentPlayer)
      throw new Error(`Please wait for your chance ${player}`);

    const givenPlayerSymbol = this.playerSymbols[player];
    const pieceTypeSymbol = getPieceType(piece);
    if (givenPlayerSymbol !== pieceTypeSymbol)
      throw new Error(`Please move your piece ${player}`);

    if (!currPiecePos) throw new Error(`Please move valid piece ${player}`);

    const validPos = getValidPos(this.chessBoard, piece, currPiecePos);

    if (!validPos.includes(moveIdx))
      throw new Error(`Please move at correct place ${player}`);

    setPos(this.chessBoard, moveIdx, piece);
    setPos(this.chessBoard, currPiecePos, "");
    this.isFinalResult();
